<!-- homepage lol -->
	
	<!-- Navbar -->
		
	<nav class="navbar navbar-custom navbar-fixed-top">
		<div class="navbar-header" id="logo" >
				<img src="<?php echo base_url('images/logo.png')?>">
				<button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#myNavbar">
					<span class="icon-bar"></span>
					<span class="icon-bar"></span>
					<span class="icon-bar"></span> 
				</button>
		</div>	
		
		<div class="collapse navbar-collapse" id="myNavbar">
		
			<ul class="nav navbar-nav navbar-right">
				
				<li><a href="#" style="margin-right: 20px;"><span class="glyphicon glyphicon-log-in"></span> Login</a></li>
				<li><a href="#"> Join Us</a></li>
			</ul>
		</div>
	</nav>
	
	<div class="container-fluid pagee" style="background-image: url('images/bg.png'); min-height: 500px; background-attachment: fixed; background-position: center; background-repeat: no-repeat; background-size: cover; ">
		
		<!-- NEED HELP? -->
		
		<div class="row">
		<div id="myCarousel" class="carousel slide" data-ride="carousel">
		
			<ol	class="carousel-indicators">
				<li data-target="#myCarousel" data-slide-to="0" class="active"></li>
				<li data-target="#myCarousel" data-slide-to="1" ></li>
				<li data-target="#myCarousel" data-slide-to="2" ></li>
			</ol>
			
			<div class="carousel-inner">
				<div class="item active">
					<img src="<?php echo base_url('images/pic1.jpg')?>" alt="">
				</div>
				
				<div class="item">
					<img src="<?php echo base_url('images/pic2.jpg')?>" alt="">
				</div>
				
				<div class="item">
					<img src="<?php echo base_url('images/pic3.jpg')?>" alt="">
				</div>
			</div>
			
			
			</div>
		</div>
		
		<!-- jobs -->
		
		<div class="row ">
			
			<div class="col-md-1 ">
			</div>
			
			<div class="col-md-10 " id="jobs">
			<br />
				
				<div class="row text-center " >
					<div class="col-md-4">
					</div>
					<div class="col-md-4">
						<h3>Browse.Talk.Hire</h3>
					</div>
					<div class="col-md-4">
					</div>
				</div>
				<br />
				
				
				<!-- search bar -->
				
				<div class="row text-center " id="search">
					<form action="" method="get">	
						<input type="text" name="search" class="search"  value="Find manpower ... " /> 
						<input type="submit" value="Search" />
					</form>
				</div>
				
				<!-- categories -->
				
				<div class="row text-left">
					<div class="col-md-3">
						<h3>In-demand jobs: </h3>	
					</div>
					<div class="col-md-9"> 
					</div>
				</div>
				<br />
				
				<div class="row text-center" id="border">
					<div class="col-md-1">
					</div>
					<div class="col-md-2">
						<a href=""><img src="<?php echo base_url('images/bartender.png')?>" alt=""></a>	
						<h4> BARTENDER <span class="badge" >5</span></h4>
					</div>
					<div class="col-md-3">
						<a href=""><img src="<?php echo base_url('images/bartender.png')?>" alt=""></a>	
						<h4> BARTENDER <span class="badge" >5</span></h4>
					</div>
					<div class="col-md-3">
						<a href=""><img src="<?php echo base_url('images/bartender.png')?>" alt=""></a>	
						<h4> BARTENDER <span class="badge" >5</span></h4>
					</div>
					<div class="col-md-2">
						<a href=""><img src="<?php echo base_url('images/bartender.png')?>" alt=""></a>	
						<h4> BARTENDER <span class="badge" >5</span></h4>
					</div>
					<div class="col-md-1">
					</div>
				
				</div>
				
				
				
				<div class="row text-center">
					
					<br /> <br />
					<div class="col-md-1">
					</div>
					<div class="col-md-2">
						<a href=""><img src="<?php echo base_url('images/bartender.png')?>" alt=""></a>	
						<h4> BARTENDER <span class="badge" >5</span></h4>
						<br /> <br />
						<a href=""><img src="<?php echo base_url('images/gardener.png')?>" alt=""></a>
						<h4> GARDENER <span class="badge">5</span></h4>
						<br /> <br />
						<a href=""><img src="<?php echo base_url('images/nanny.png')?>" alt=""></a>
						<h4> NANNY <span class="badge">5</span></h4>
						<br /> <br />
					</div>
					<div class="col-md-3">
						<a href=""><img src="<?php echo base_url('images/carpenter.png')?>" alt=""></a>
						<h4> CARPENTER <span class="badge">5</span></h4>
						<br /> <br />
						<a href=""><img src="<?php echo base_url('images/janitor.png')?>" alt=""></a>
						<h4> JANITOR <span class="badge">5</span></h4>
						<br /> <br />
						<a href=""><img src="<?php echo base_url('images/plumber.png')?>" alt=""></a>
						<h4> PLUMBER <span class="badge">5</span></h4>
						<br /> <br />
					</div>
					<div class="col-md-3">
						<a href=""><img src="<?php echo base_url('images/cook.png')?>" alt=""></a>
						<h4> COOK <span class="badge">5</span></h4>
						<br /> <br />
						<a href=""><img src="<?php echo base_url('images/maid.png')?>" alt=""></a>
						<h4> MAID <span class="badge">5</span></h4>
						<br /> <br />
						<a href=""><img src="<?php echo base_url('images/tutor.png')?>" alt=""></a>
						<h4> TUTOR <span class="badge">5</span></h4>
						<br /> <br />
					</div>
					<div class="col-md-2">
						<a href=""><img src="<?php echo base_url('images/driver.png')?>" alt=""></a>
						<h4> DRIVER <span class="badge">5</span></h4>
						<br /> <br />
						<a href=""><img src="<?php echo base_url('images/massause.png')?>" alt=""></a>
						<h4> MASSEUSE <span class="badge">5</span></h4>
						<br /> <br />
						<a href=""><img src="<?php echo base_url('images/waiter.png')?>" alt=""></a>
						<h4> WAITER <span class="badge">5</span></h4>
						<br /> <br />
						<br /> <br />
					</div>
					<div class="col-md-1">
					</div>
				
				</div>	
			
			</div>
			
			<div class="col-md-1">
			</div>
			
		
		</div>
		
		<!-- footer -->
		
		<div class="row" id="footer">
			
			<div class="col-lg-1">
			</div>
			<div class="col-lg-1">
				</br>
				<a>About Us</a>
				<br /><a>Comments</a>
				<br /><a>Suggestions</a>
				<br />
			</div>
			<div class="col-lg-1">
				</br>
				<a>Special Offer</a>
				<br /><a>Discount</a>
				<br /><a>Freebies</a>
				<br />
			</div>
			<div class="col-lg-6 text-center">
				<br />
				<p>hiremetro © 2017 | Designed by Group 4</p>
			</div>
			<div class="col-lg-2 ">
				<br />
				<p><img src="<?php echo base_url('images/fb.ico')?>" alt=""> facebook.com/hiremetro</p>
				<p><img src="<?php echo base_url('images/twitter.png')?>" alt=""> @hiremetro</p>
				<p><img src="<?php echo base_url('images/gmail.ico')?>" alt=""> hiremetro@gmail.com</p> 
			</div>
			<div class="col-lg-1">
			</div>
			
		</div>
		
	</div>